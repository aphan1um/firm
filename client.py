...
class Environment():
    ...
    def new_step(self, cpu_action, mem_action, io_action, net_action, id):
        action = message_pb2.Action(cpu=cpu_action, memory=mem_action, llc=0, io=io_action, network=net_action)
        ...
        reward = NUM_RESOURCES*self.slo_retainment + self.curr_cpu_util/self.cpu_limit + self.curr_mem_util/self.mem_limit + self.curr_io_util/self.io_limit + self.curr_net_util/self.net_limit
        # if not done:
        #    reward = -(NUM_RESOURCES)*(1-self.slo_retainment) + self.curr_cpu_util/self.cpu_limit + self.curr_mem_util/self.mem_limit + self.curr_io_util/self.io_limit + self.curr_net_util/self.net_limit
